...
class DisclinationLine(ClassBase):
    ...
    __properties__ = {
        **(ClassBase.__properties__),
        "smooths": "Read-only: All generated smoothed versions of this disclination line.",
        "smooth": "Read-only: The latest generated smoothed version, if any.",
        "kind": "Read-only: Shorthand for the end-to-end topology kind of this line.",
    }
...
class DisclinationLineSmooth(SmoothedLine):
    ...
    - `_calc_coords`: the index-space coordinates currently entering the smoothing
      pipeline, including any temporary periodic padding.
    - `_calc_result`: the final smoothed defect-line indices in lattice space.
    - `_calc_result_coords`: the corresponding transformed real-space result.
    - `_calc_padding_num`: temporary padding length used for cross-boundary lines.
    ...
    def _helper_commit_apply_opts_main(
        ...
        if not self._state_is_smoothed:
            object.__setattr__(
                self, "_calc_result", self.owner._raw_defect_indices.copy()
            )
...
class DisclinationLineSmoothPlot(HostBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self_kwargs = {
            k: kwargs.pop(k) for k in list(kwargs.keys()) if k in self_descriptions
        }
    ...
    def _helper_get_coords(self, logger=None):
        ...
        if not is_wrap:
            ...
            # Adjacent defect indices are expected to move by at most one lattice step in index space; larger jumps are treated as periodic-wrap breaks.
...
# `PlaneGridPolar`. If pose resolution changes, keep `_calc_normal` and the
...
class DefectSectionGrid(HostBase):
    ...
    - `_calc_normal`: the resolved normal currently driving the wrapped section.
    - `_impl_normals`: registered named normal providers for this section.
    ...
    def _helper_check_state_normal(self, state_normal, desc):
        if isinstance(state_normal, str):
            state_normal = as_str(
                state_normal, name="The normal selector of defect section grid"
            )
            ...
            return state_normal
        return as_Vect(
            state_normal, name="The direct normal of defect section grid", is_norm=True
        )
    ...
    def _helper_resolve_normal(self, tangent):
        ...
        if isinstance(normal, str):
            ...
            if callable(normal):
                normal = normal()
        normal = as_Vect(
            normal, name="The resolved normal of defect section grid", is_norm=True
        )
    ...
    def act_register_normal(self, key, value, logger=None):
        ...
        try:
            value = as_Vect(
                value, name=f"The normal {key!r} of defect section grid", is_norm=True
            )
        except Exception:
            logger.warning(
    ...
    def show_normals(self, is_return=False, logger=None):
        is_return = as_bool(
            is_return, name="Whether to return the normal summary", replace=False
        )
