...
def _boy_polynomial(n: np.ndarray) -> np.ndarray:
    ...
    p[..., 1] = (7.0 / 8.0) * ((y2 - z2) + z * x * (z2 - x2) + x * y * (y2 - x2))
    p[..., 2] = (
        (1.0 / 8.0)
        * (x + y + z)
...
def director_color_pareto_oklab_055(n: np.ndarray) -> np.ndarray:
    """Map directors to RGB with the selected vivid OKLab solution at J_loc=0.55."""
    A = np.array(
        [
