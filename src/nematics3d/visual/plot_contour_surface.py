...
from nematics3d.datatypes import UNSET, Unset, as_ColorRGB, as_bool, as_number, as_str
...
class OptsContourSurface(OptsGlyph):
    """Visual configuration for one extracted contour surface."""
    ...
    impl_validators: ClassVar[Mapping[str, Any]] = {
        **dict(OptsGlyph.impl_validators),
        ...
        "line_width": lambda v, d: as_number(v, name=d, value_range=(0.0, np.inf)),
...
class PlotContourSurface(PlotGlyph):
    """Render one extracted :class:`ContourSurface` mesh in a figure."""
...
__all__ = ["OptsContourSurface", "PlotContourSurface"]
