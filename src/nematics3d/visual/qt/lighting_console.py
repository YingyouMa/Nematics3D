...
class LightingConsole(QtWidgets.QWidget):
    """Standalone lighting editor bound to one glyph-like host."""
    ...
    def __init__(self, host, parent_panel):
        ...
        if not hasattr(host, "opts"):
            missing.append("opts")
        if missing:
            raise TypeError(
                "LightingConsole requires a lighting-compatible host. "
                f"Missing: {missing}"
            )
        ...
        slider_throttle_ms = getattr(parent_panel, "slider_throttle_ms", 20)
        self._slider_throttle = UIThrottle(interval_ms=slider_throttle_ms, parent=self)
    ...
    def closeEvent(self, event):
        try:
            ...
            if getattr(self.parent_panel, "_lighting_console", None) is self:
