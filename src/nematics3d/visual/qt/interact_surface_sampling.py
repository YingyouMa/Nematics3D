...
from nematics3d.sample import SurfaceSampling
from nematics3d.sample.surface_sampling import _helper_resolve_spacing_for_target_count
...
class InteractSurfaceSampling(PanelBase):
    ...
    def build_ui(self):
        ...
        self.chk_is_auto_spacing = QtWidgets.QCheckBox(
            "Automatic spacing from default_sample_count_target", group_spacing
        )
        ...
        self.sliders["spacing"] = make_labeled_slider_row(
            ...
            state_key="spacing",
            value_min=1.0e-3,
            value_max=max(1.0, spacing_current * 5.0),
        ...
        self.input_default_target.setValue(int(self.host.opts.default_sample_count_target))
        ...
        self.lbl_apply_hint = QtWidgets.QLabel(
            "Surface sampling is not updated live. Adjust parameters first, then click Apply once.",
            group_apply,
        )
    ...
    def _helper_get_display_spacing(self) -> float:
        ...
        if area > 0.0:
            return max(float(_helper_resolve_spacing_for_target_count(area, target)), 1.0e-12)
        return 1.0

    def _update_spacing_mode_label(self):
        self.lbl_spacing_mode.setText(
            f"Current effective spacing: {self._helper_get_display_spacing():.3f}"
        )
    ...
    def _update_spacing_estimate_label(self):
        spacing = self._helper_get_preview_spacing()
        area = float(getattr(self.host, "calc_surface_area", 0.0))
        if area <= 0.0:
            ...
            return
        estimate = max(int(np.round(area / ((np.sqrt(3.0) / 2.0) * spacing**2))), 1)
        self.lbl_spacing_estimate.setText(
            f"Estimated sample count for current spacing: {estimate}"
        )

    def _update_summary_labels(self):
        area = float(getattr(self.host, "calc_surface_area", 0.0))
        surface_clean = getattr(self.host, "calc_surface_clean", None)
        triangles = 0 if surface_clean is None else int(surface_clean.n_cells)
        samples = len(np.asarray(getattr(self.host, "calc_sample_points", ())))
        self.lbl_surface_area.setText(f"Surface area: {area:.6g}")
        self.lbl_triangle_count.setText(f"Current triangle count: {triangles}")
        self.lbl_sample_count.setText(f"Current sampled point count: {samples}")
    ...
    def commit(self):
        ...
        spacing = None if self.state["is_auto_spacing"] else float(self.state["spacing"])
    ...
    def _sync_func(self, **kwargs):
        ...
        self._sync_from_host_slider("spacing", self._helper_get_display_spacing())
        self.sliders["spacing"].set_enabled(not is_auto)
        if "seed" in kwargs:
            self._sync_spinbox(self.input_seed, self.host.opts.seed)
        if "oversample" in kwargs:
            self._sync_spinbox(self.input_oversample, self.host.opts.oversample)
        if "relax_steps" in kwargs:
            self._sync_spinbox(self.input_relax_steps, self.host.opts.relax_steps)
        if "k_neighbors" in kwargs:
            self._sync_spinbox(self.input_k_neighbors, self.host.opts.k_neighbors)
        if "default_sample_count_target" in kwargs:
            self._sync_spinbox(self.input_default_target, self.host.opts.default_sample_count_target)
        self._update_spacing_mode_label()
        self._update_summary_labels()
        self._helper_update_first_point_marker()
...
__all__ = ["InteractSurfaceSampling"]
