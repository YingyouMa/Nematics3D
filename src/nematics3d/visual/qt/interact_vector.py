...
from nematics3d.visual.qt.panel_base import make_labeled_slider_row, LogTickMapper
...
class InteractVector(InteractGlyphBase):
    ...
    def _extra_commit(self, params):
        params["length"] = self._helper_scale_resolver_value(
            self._length_rescale_base,
            self.state["length_rescale"],
        )
    ...
    def _sync_func(self, **kwargs):
        ...
        if "length" in kwargs and not getattr(self, "_is_gui_updating", False):
            self._helper_reset_rescale_control(
                "length_rescale", "length", "_length_rescale_base"
            )
