...
class ScalarBarRegistry(RegistryBase):
    ...
    _FONT_FAMILY_INT_TO_NAME = {0: "arial", 1: "courier", 2: "times"}

    def _helper_build_scalar_bar_rebuild_signature(self, scalar_bar):
        return {"is_interactive": scalar_bar.opts.is_interactive}

    def _helper_resolve_scalar_bar(self, key):
        scalar_bar = key if isinstance(key, ScalarBar) else self[key]
    ...
    def _helper_resolve_scalar_bar_mapper(self, scalar_bar):
        ...
        if mapper is not None:
            if not hasattr(mapper, "lookup_table") and hasattr(
                ...
                mapper_scalar_bar = getattr(scalar_bar, "entity_mapper_adapter", None)
                if mapper_scalar_bar is not None:
                    mapper_scalar_bar.dataset = source.calc_poly
                ...
                scalar_bar._helper_set_mapper_adapter(mapper_scalar_bar)
                return mapper_scalar_bar
            return mapper
    ...
    def _helper_resolve_scalar_bar_source_display(self, scalar_bar):
        source = getattr(scalar_bar, "source", None)
        if source is None:
            return {"title": scalar_bar.name, "cmap": None, "clim": None}
    ...
    def _helper_resolve_scalar_bar_widget(self, scalar_bar):
        ...
        return None if widgets is None else widgets.get(backend_key)

    def _helper_configure_scalar_bar_widget(self, scalar_bar):
        widget = self._helper_resolve_scalar_bar_widget(scalar_bar)
        widget_old = getattr(scalar_bar, "backend_widget", None)
        observer_tag_old = getattr(scalar_bar, "impl_backend_widget_observer_tag", None)
        if widget_old is not None and observer_tag_old is not None:
            try:
                widget_old.RemoveObserver(observer_tag_old)
    ...
    def _helper_detach_scalar_bar_widget_observer(self, scalar_bar):
        widget = getattr(scalar_bar, "backend_widget", None)
        observer_tag = getattr(scalar_bar, "impl_backend_widget_observer_tag", None)
        if widget is not None and observer_tag is not None:
            try:
                widget.RemoveObserver(observer_tag)
            except (AttributeError, RuntimeError, ReferenceError):
                pass
        scalar_bar._helper_set_backend_widget_observer_tag(None)
        return observer_tag
    ...
    def _helper_pull_scalar_bar_widget_geometry(self, scalar_bar):
        ...
        payload = {"position": position, "width": size[0], "height": size[1]}
    ...
    def _helper_remove_scalar_bar_backend(self, scalar_bar):
        self._helper_detach_scalar_bar_widget_observer(scalar_bar)
        backend_key = self._helper_find_scalar_bar_backend_key(scalar_bar)
    ...
    def _helper_create_scalar_bar_backend(self, scalar_bar):
        ...
        kwargs_create.update(title=scalar_bar.impl_name_pv, mapper=mapper, render=False)
        ...
        if backend is None:
            raise RuntimeError(
                f"Failed to create or resolve PyVista scalar-bar backend for {scalar_bar!r}."
            )
