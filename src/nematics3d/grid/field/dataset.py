...
class GridFieldDataset(ClassBase):
    ...
    # Numerical operators stay on the dataset API; implementation is delegated
    # to focused sibling modules instead of inheritance-only mixins.
    _helper_first_derivative_index = _derivatives._helper_first_derivative_index
    _helper_second_derivative_index = _derivatives._helper_second_derivative_index
    _helper_physical_direction_weights = _derivatives._helper_physical_direction_weights
    _helper_spatial_derivative_info = _derivatives._helper_spatial_derivative_info
    _helper_spatial_derivative_result = _derivatives._helper_spatial_derivative_result
    _helper_vector_gradient_split = _derivatives._helper_vector_gradient_split
    act_gradient = _derivatives.act_gradient
    act_derivative = _derivatives.act_derivative
    act_second_derivative = _derivatives.act_second_derivative
    act_divergence = _derivatives.act_divergence
    act_tensor_divergence = _derivatives.act_tensor_divergence
    act_directional_derivative = _derivatives.act_directional_derivative
    act_curl = _derivatives.act_curl
    act_tensor_curl = _derivatives.act_tensor_curl
    act_elastic_deformation = _derivatives.act_elastic_deformation
    act_strain_rate_and_vorticity_tensor = (
        _derivatives.act_strain_rate_and_vorticity_tensor
    )
    act_laplacian = _derivatives.act_laplacian
    act_componentwise_laplacian = _derivatives.act_componentwise_laplacian
    _helper_as_gaussian_sigma_3 = _smoothing._helper_as_gaussian_sigma_3
    _helper_as_gaussian_boundary_mode = _smoothing._helper_as_gaussian_boundary_mode
    _helper_as_gaussian_weights = _smoothing._helper_as_gaussian_weights
    _helper_gaussian_smooth_info = _smoothing._helper_gaussian_smooth_info
    _helper_gaussian_smooth_result = _smoothing._helper_gaussian_smooth_result
    _helper_gaussian_smooth_values = _smoothing._helper_gaussian_smooth_values
    act_gaussian_smooth = _smoothing.act_gaussian_smooth
