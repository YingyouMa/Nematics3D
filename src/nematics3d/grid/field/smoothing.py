...
if TYPE_CHECKING:
    from .dataset import GridFieldDataset
...
def act_gaussian_smooth(
    ...
    *,
    ...
    if weights_values is None:
        ...
        if np.all(valid):
            smoothed_values = smoothed_weighted_values
            denominator = smoothed_weights
            if values.ndim > 3:
                denominator = denominator.reshape(
                    denominator.shape + (1,) * (values.ndim - 3)
                )
            smoothed_values /= denominator
        else:
            smoothed_values = values.copy()
            if values.ndim > 3:
                valid_weights = smoothed_weights[valid].reshape(
