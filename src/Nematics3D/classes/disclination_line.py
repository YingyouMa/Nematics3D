...
class DisclinationLine(ClassBase):
    ...
    Normal users can inspect the raw line by iterating over the object, call
    `act_smooth()` to create smoothed versions, and use `act_visualize()` to
    plot the line. The latest smoothed result is available through `smooth`,
    while all generated smooth versions are available through `smooths`.
    """
...
class DisclinationLineSmooth(SmoothedLine):
    ...
    It stores the smoothed lattice trajectory, the corresponding real-space
    coordinates, and any derived visualization or cross-section helpers.
    Normal users usually obtain it through the line smoothing helper,
    access the current one-to-one visualization through the visual relation, and
    then use the visualize or cross-section helpers on the returned object.
    """
...
class DisclinationLineSmoothPlot(HostBase):
    ...
    It wraps an internal `PlotTube` and exposes two high-level controls:
    whether visualization should use the smoothed geometry and whether
    periodic wrapping should be applied before plotting. Normal users usually
    obtain it through `DisclinationLineSmooth.act_visualize()`.
    """
...
class DefectSectionGrid(HostBase):
    ...
    It resolves a section origin from the line position, chooses a section
    normal from either the tangent, a registered named normal, or a direct
    vector, and then drives a wrapped `PlaneGridPolar`. Normal users usually
    obtain it through `DisclinationLineSmooth.act_cross_section()`.
    """
