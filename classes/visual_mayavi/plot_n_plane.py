...
class PlotnPlane():
    ...
    def make_figure(
        ...
        if is_n_defect:
            ...
            axis_both = np.array([self.plane._axis1, np.cross(self.plane._normal, self.plane._axis1)])
            ...
            defect_vicinity_index = defect_vicinity_grid(defect_plane_index, num_shell=1).reshape((-1,3))[:, 1:]
            bulk_index, defect_vicinity_index = split_points(self.plane._grid_int, defect_vicinity_index)
            
            defect_vicinity = np.einsum('ai, ib -> ab', defect_vicinity_index, axis_both) * self.plane._space1 + self.plane._offset
            defect_vicinity = select_grid_in_box(defect_vicinity, corners_limit)
            
            bulk = np.einsum('ai, ib -> ab', bulk_index, axis_both) * self.plane._space1 + self.plane._offset
