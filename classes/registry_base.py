...
class RegistryBase:
    ...
    def _helper_check_name(self, name: str, logger=None):
        ...
        if name_input in name_set:
            ...
            while new_name in name_set:
                ...
                index += 1
            logger.warning(f"{name_input!r} already exists in Registry {self.name!r}! Renamed to {new_name!r}.")
            name = new_name
        return name 
    ...
    def __getitem__(self, key: str | int | None):
        if key is None:
            ...
            for obj in self._entity:
                if obj.name == key:
                    return obj
            raise KeyError(f"No object with name '{key}' found in Registry {self.name!r}.")
        else:
            raise TypeError(
    ...
    def _helper_repr_by_order(self, is_name=True) -> str:
        ...
        for i, name in enumerate(names):
            lines.append(
                f"{i:>{idx_width}d}:       {name:<{name_width}}"
            )
