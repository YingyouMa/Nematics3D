...
class FigureCamera:
    ...
    @property
    def position(self): return self._internal_cam.position
    @position.setter
    def position(self, v): 
        self._internal_cam.position = as_Vect(v, name="The position of the camera", dim=3)
        self._internal_plotter.render()

    @property
    def view_up(self): return self._internal_cam.up
    @view_up.setter
    def view_up(self, v): 
        self._internal_cam.up = as_Vect(v, name="The upward direction of the camera", dim=3)
        self._internal_plotter.render()
    ...
    def act_view_xy(self):
        ...
        self._helper_sync_from_cam() # 动作执行后必须同步 opts
    ...
    def act_log_parameters(self, is_return: bool = False, logger=None) -> str:
        ...
        lines.append(f"  position: {self.position!r}  # The absolute coordinate of the camera")
        lines.append(f"  view_up : {self.view_up!r}  # The upward orientation of the camera")
