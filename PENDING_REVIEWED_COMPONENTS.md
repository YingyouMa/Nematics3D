...
- Source: `src/nematics3d/geometry/points.py`.
- Status: reviewed and moved into the canonical `geometry` package under the clearer name `points_membership_mask()`; the old `mark_points_membership()` name and its `general` compatibility path were removed completely, and `QPlane` now imports and calls the canonical geometry helper directly.
- Behavior reviewed: exact row-wise point membership; arbitrary coordinate dimension; finite real inputs; empty point or candidate collections; duplicate candidate rows; preservation of one boolean result per input point; mixed numerical dtypes promoted to a common dtype before comparison; exact floating-point comparison without tolerance; and numerical equality of signed zero.
- Fixes and cleanup: replaced the old dtype-sensitive raw-byte row comparison with structured numerical row comparison after common-dtype promotion, eliminating incorrect behavior when the two inputs used different item sizes and avoiding byte-level distinctions such as `+0.0` versus `-0.0`.
- Focused tests: `tests/geometry/test_points.py`, covering basic membership, mixed integer dtypes, integer/float promotion, duplicate candidates, empty inputs, exact floating comparison, signed zero, dimensional errors, and non-finite inputs.
