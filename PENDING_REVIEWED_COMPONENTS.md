...
- Review commits: structured implementation `460dd7d`; canonical geometry export `5fd81d7`; removal from `geometry/misc.py` `f56fc3d`; focused tests `688fce4`; numerical tolerance cleanup `af93c71`; direct `DisclinationLine` migration `5a81619`; removal of the private compatibility adapter and metric flag `15fbd26`; legacy-API regression test update `57c7142`.
- Archive note: implementation, structured output, caller migration, API cleanup, and focused tests are complete. Keep this component pending until focused/downstream pytest plus Black/compile/import validation are actually run and an exact final reviewed commit is recorded.
