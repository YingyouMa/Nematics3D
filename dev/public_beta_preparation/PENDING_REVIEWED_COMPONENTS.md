...
- Visualization cleanup: contour mesh materialization now reuses the same internal point-display-data helper as `PlotPolyData`, eliminating the duplicated deep-copy/`rgba`/`opacity`/`scalars` assembly path while keeping contour topology sourced dynamically from the current extracted mesh.
- Interaction cleanup: `InteractPolyData` exposes its edge-controls block as a reusable implementation with a configurable edge-width option name. `InteractContourSurface` now subclasses `InteractPolyData`, so contour surfaces receive the existing show-edges/color/width controls in addition to the contour-level slider. The contour API keeps its existing `line_width` name; ordinary PolyData keeps `edge_width`.
...
- Refresh-chain regression: a real off-screen VTK test now changes a radial contour level across meshes with different topology and verifies that the owner-driven refresh updates `raw_coords` and the live actor mapper dataset while preserving color, opacity, edge visibility/color, and the legacy contour `line_width` option. A second regression verifies that mesh-mode `Bounds` clipping is reapplied after the contour level changes.
- Validation: focused contour-domain tests pass 15 tests. Focused contour/PolyData/interaction/scalar-bar visual regression passes 34 tests. Full `tests/visual` passes 174 tests with the single pre-existing `test_plot_vector_debug_figure` return-value warning. Black and Ruff pass for the touched contour/PolyData visual files; Windows VTK `wglMakeCurrent` cleanup stderr remains environmental noise.
